...
export class Clientele {
  ...
  public ajouterCa(nom: string, prenom: string, ca: number): void {
    ...
    if (clientListe instanceof Client) {
      const type = ClientPrivilegie.typesPrivileges.getPrivilege(clientListe.ca);
      if (type !== null) {
        const cp = new ClientPrivilegie(clientListe);
        this._listeClients[idx] = cp;
      }
    }
  }
